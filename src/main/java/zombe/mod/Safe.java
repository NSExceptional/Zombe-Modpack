...
public final class Safe extends ZMod {
    ...
    private static Color optDangerColor, optDangerColorSun;
    private static boolean optShowWithSun;
    private static int optLookupRadius;

    private static final int SAFE_MAX = 2048;
    private static Vec3i safePoses[];
    private static Color safeMarks[];
    ...
    public Safe() {
        ...
        addOption("keySafeShow", "Show / hide un-safe markers", Keyboard.KEY_L);
        addOption("keySafeGhost", "Toggle show marks through walls", Keyboard.KEY_K);
        addOption("optSafeLookupRadius", "Un-safe lookup radius", 16, 0, 64);
        addOption("optSafeShowWithSun", "Mark 'safe at midday' differently", true);
        addOption("optSafeDangerColor", "Marks color", "0xff0000");
        addOption("optSafeDangerColorSun", "Marks color (sun)", "0xdddd00");
    }
    ...
    protected void onClientTick(EntityPlayerSP player) {
        if (isInMenu()) return;
        if (wasKeyPressedThisTick(keyShow))  safeShow  = !safeShow;
        if (wasKeyPressedThisTick(keyGhost)) safeGhost = !safeGhost;
    }

...
public final class Cloud extends ZMod {
    ...
    public Cloud() {
        ...
        registerHandler("getCloudHeight");
        registerListener("beforeRenderClouds");
        registerListener("afterRenderClouds");
        ...
        addOption("keyCloudUp", "Move clouds up", Keyboard.KEY_NONE);
        addOption("keyCloudDown", "Move clouds down", Keyboard.KEY_NONE);
        addOption("optCloudOffset", "Cloud offset", 4f, -60f, 140f);
    }
    ...
    protected void onClientTick(EntityPlayerSP player) {
        ...
        if (wasKeyPressedThisTick(keyVanilla)) {
            ...
            if (cloudVanilla) setCloudSetting(vanillaShow);
        }
        if (wasKeyPressedThisTick(keyToggle)) {
            if (cloudVanilla) cloudVanilla = false;
            else cloudShow = !cloudShow;
        }
    ...
    private static Entity view = null;
